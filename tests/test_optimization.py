...
class TestOptimization(unittest.IsolatedAsyncioTestCase):
    ...
    def test_shadow_price_extraction(self):
        ...
        opt_res = self.opt.perform_dayahead_forecast_optim(
            df_input_data_dayahead, self.p_pv_forecast, self.p_load_forecast
        )
    ...
    def test_shadow_price_relaxed_lp_fallback(self):
        ...
        df_input_data = self.prepare_forecast_data()
        p_pv = self.p_pv_forecast.values.ravel()
        p_load = self.p_load_forecast.values.ravel()
        unit_load_cost = df_input_data[self.fcst.var_load_cost].values
        unit_prod_price = df_input_data[self.fcst.var_prod_price].values
