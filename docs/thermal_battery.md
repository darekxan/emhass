...
* **shgc**: Solar Heat Gain Coefficient, fraction of solar radiation passing through windows (default: 0.6).
    ...
    * Example: `0.6`
    * When `ghi` is available, these gains reduce residual heating demand and directly warm the thermal battery state used for predicted temperature
...
5. **sensor.solar_gain{k}** - Passive solar gain applied to the thermal battery (kWh per timestep) — meaningful when `ghi`, `window_area`, and `shgc` are configured
...
Behavior note:
- Solar gains now work in two places: they reduce residual `heating_demand` **and** directly add heat into the thermal battery state behind `temp_predicted`
- This means strong sunshine can now increase predicted temperature instead of only causing stagnation
- The model still uses raw `ghi` as a proxy for transmitted solar radiation, so orientation, shading, tilt, and frame losses are not explicitly modeled. If gains look too strong or weak, calibrate `window_area` and `shgc` conservatively
